...
export const getAllAchievements = async (params?: { search?: string, difficulty?: string, order?: string }) => {
...
export const getAchievementSortBy = async (sortBy: string, orderBy: string) => {
    const res = await api.get(`/api/achievements/sort?sortBy=${sortBy}&order=${orderBy}`);
    return res.data;
}
