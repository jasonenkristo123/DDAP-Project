        ...
        <div className="w-full p-4 md:p-8 lg:p-15">
            ...
            {loading ? (
                <div className="mt-6 flex justify-center">
                    <p className="text-brownbold font-manrope-600">Loading achievements...</p>
                </div>
            ) : achievements.length === 0 ? (
                <div className="mt-6 flex justify-center">
                    <p className="text-brownbold font-manrope-600">No achievements found.</p>
                </div>
            ) : (
                achievements.map((data, index) => (
                    <div key={data.id_achievement || index} className={`mt-6 ${!data.is_unlocked ? 'opacity-50' : ''}`}>
                        ...
                        <div className="flex items-center gap-6 mt-4">
                            <div className="relative border border-brownbold rounded-full w-16 h-16 overflow-hidden bg-gray-200 shrink-0">
                                {data.icon_url && <Image src={data.icon_url} alt={data.title} fill className="object-cover" />}
                            </div>
                            <div>
                                <div className="flex items-center gap-3 flex-wrap">
                                    ...
                                    {!data.is_unlocked && (
                                        <div className="border-2 border-brownbold bg-gray-300 px-3 py-0.5 rounded-full">
                                            <p className="text-xs font-manrope-600 text-gray-700">Locked</p>
                                        </div>
                                    )}
                                </div>
